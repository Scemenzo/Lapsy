...
public class VideoConverter {
    ...
    public void convertVideo(String inputPath, String outputPath, double fps, double duration) {
        ...
        if (!inputVideo.isOpened()) {
            System.out.println("Non è stato possibile aprire il video in ingresso");
            return;
        }
        ...
        if (!outputVideo.isOpened()) {
            System.out.println("Non è stato possibile aprire il video in uscita\n" +
                    "FOURCC: " + inputVideo.get(Videoio.CAP_PROP_FOURCC) + "\n" +
                    "Size: " + inputVideo.get(Videoio.CAP_PROP_FRAME_WIDTH) + "," + inputVideo.get(Videoio.CAP_PROP_FRAME_HEIGHT));
            return;
        }
        ...
        for (double icnt = frameCountRatio; icnt < inputFrameCount; icnt += frameCountRatio) {
            ...
            while (framePicker < Math.floor(icnt)) {
                inputVideo.grab();
                framePicker++;
            }
